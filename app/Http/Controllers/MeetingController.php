...
class MeetingController extends Controller
{
    ...
    public function index(UserController $user, Request $request) {
        ...
        return view('meeting.index', compact('info'));
    }

    public function allMeetings(UserController $user, Request $request) {
        
        $info = $user->getCurrentUser($request);

        if($request->ajax()) {
            $data = Post::where('status', 1)->where('isConfirm', 1)->where('type', 'Meeting')->get();
            ...
            if($info['info']['access_level'] === 'Intern') {
               ...
               $implementor = array();
                ...
                $this->meeting = count($container);
                return DataTables::of($container)->make(true);
            } else {

                $this->meeting = count($data);
