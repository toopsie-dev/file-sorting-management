...
class ProcessController extends Controller
{
    ...
    public function allProcess(UserController $user, Request $request) {
        
        $info = $user->getCurrentUser($request);

        if($request->ajax()) {
            $data = Post::where('status', 1)->where('isConfirm', 1)->where('type', 'Process')->get();
            ...
            if($info['info']['access_level'] === 'Intern') {
               ...
               $implementor = array();
                ...
                $this->process = count($container);
                return DataTables::of($container)->make(true);
            } else {
                $this->process = count($data);
